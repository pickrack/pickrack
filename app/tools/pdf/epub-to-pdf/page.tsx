...
export default function EPUBtoPDFPage() {
  ...
  const convert = async () => {
    ...
    try {
      ...
      setDownloadName(files[0].file.name.replace(/\.epub$/i, ".pdf"));
    } catch (e) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-12">
      <div className="text-center mb-8">
        <h1 className="text-3xl sm:text-4xl font-bold tracking-tight">EPUB to PDF</h1>
      ...
      <ToolUploadZone
        ...
        label="Drop an EPUB here or click to choose"
        sublabel="Single .epub file up to 50MB"
      />
        ...
        <div className="mt-6 rounded-2xl border bg-white p-5" style={{ borderColor: "var(--color-border)" }}>
          ...
          >
            {processing ? (
              ...
              <><BookOpen className="w-4 h-4" /> Convert to PDF</>
            )}
          </button>
        </div>
      )}
