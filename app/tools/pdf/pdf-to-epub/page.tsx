...
export default function PDFtoEPUBPage() {
  ...
  const [downloadName, setDownloadName] = useState<string>("converted.epub");
  ...
  const convert = async () => {
    ...
    try {
      ...
      setDownloadName(files[0].file.name.replace(/\.pdf$/i, ".epub"));
    } catch (e) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-12">
      <div className="text-center mb-8">
        <h1 className="text-3xl sm:text-4xl font-bold tracking-tight">PDF to EPUB</h1>
        <p className="mt-3 text-gray-600">
          Convert PDF to EPUB ebook for Kindle, Kobo, Apple Books, or any reader. Server-side Calibre engine. Best for text-based PDFs.
        </p>
      </div>
      ...
      <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 flex items-start gap-3 text-sm text-amber-900">
        ...
        <div>
          ...
          <p className="mt-1 text-amber-800">
            Your PDF uploads over HTTPS, gets converted by Calibre, then deleted immediately. Scanned PDFs need OCR first — text-only conversion supported.
          </p>
        </div>
      </div>
