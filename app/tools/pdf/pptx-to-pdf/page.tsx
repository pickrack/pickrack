...
import { Loader2, Download, Presentation, AlertTriangle } from "lucide-react";
...
export default function PPTXtoPDFPage() {
  ...
  const convert = async () => {
    ...
    try {
      ...
      setDownloadName(files[0].file.name.replace(/\.(pptx|ppt|odp)$/i, ".pdf"));
    } catch (e) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-12">
      <div className="text-center mb-8">
        <h1 className="text-3xl sm:text-4xl font-bold tracking-tight">PowerPoint to PDF</h1>
      ...
      <ToolUploadZone
        ...
        label="Drop a presentation here or click to choose"
        sublabel="Single .pptx, .ppt, or .odp file up to 50MB"
      />
        ...
        <div className="mt-6 rounded-2xl border bg-white p-5" style={{ borderColor: "var(--color-border)" }}>
          ...
          >
            {processing ? (
              ...
              <><Presentation className="w-4 h-4" /> Convert to PDF</>
            )}
          </button>
        </div>
      )}
