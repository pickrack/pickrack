...
const intro = (
  <>
    ...
    <p>
      A password generator that sends your generated password to a server is a contradiction. A QR code generator that logs every URL you encode defeats the point. An age calculator that needs an internet connection is silly. Twelve of Pickrack&apos;s thirteen calc/generator tools run <strong>entirely in your browser</strong> — they have to, because the threat model demands it. (The one exception, the Currency Converter, fetches live mid-market rates through a server-side proxy so the API key stays off the client; the rates are cached for an hour.)
    </p>
    <p>
    ...
    <h2>The thirteen calc tools and what they&apos;re good at</h2>
    <ul>
