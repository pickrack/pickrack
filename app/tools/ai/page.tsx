...
const intro = (
  <>
    ...
    <p>
      The phrase &quot;AI tools&quot; on a free website usually means one of two things: either a wrapper around OpenAI&apos;s API that funnels your input through a third-party reseller (often with extra logging, sometimes with fingerprinting), or a custom in-house model trained on whatever the operator could scrape. Both raise questions about who sees your text, what they do with it, and how long they keep it. Pickrack&apos;s nine AI tools do something different. The six text tools <strong>call Anthropic&apos;s Claude API directly</strong> from the server, send only the text you typed (or, for Chat with PDF and Translate Document, the text extracted from your PDF inside your browser), and receive only the response. The three image tools (QR-art, upscaler, image-to-image) run on a <strong>self-hosted GPU we operate</strong> — your image never goes to a third party. Either way, there is no data broker in the middle.
    </p>
    <p>
    ...
    <h2>The nine AI tools and when to use each</h2>
    <ul>
      ...
      <li><strong><a href="/tools/ai/qr-art-generator/">QR-Art Generator</a></strong> — turn a URL or text into a scannable, artistic QR code using Stable Diffusion with a QR-Monster ControlNet, rendered on our self-hosted GPU. Pick a style prompt and model; the output is scan-verified before it&apos;s shown.</li>
      <li><strong><a href="/tools/ai/ai-image-upscaler/">AI Image Upscaler</a></strong> — upscale a photo 2× or 4× with the Swin2SR super-resolution model on our GPU. Best for small or low-resolution images; noticeably sharper than the browser-side bicubic upscaler in the image category.</li>
      <li><strong><a href="/tools/ai/ai-img2img/">Image-to-Image</a></strong> — restyle a photo from a text prompt (e.g. &quot;oil painting, thick brush strokes&quot;) using Stable Diffusion img2img on our GPU. An adjustable strength slider controls how far the result moves from the original.</li>
    </ul>
