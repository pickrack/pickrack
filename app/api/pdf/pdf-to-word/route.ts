 ...
 *
 * Converts PDF to DOCX using LibreOffice headless.
 * Best with text-based PDFs; scanned PDFs need OCR first.
 */
...
export async function POST(req: NextRequest) {
  const limited = checkRateLimit(req, "pdf-to-word", 8, 60_000);
  ...
  try {
    ...
    const outputBuf = await libreofficeConvert(inputBuf, "pdf", "docx", {
      infilter: "writer_pdf_import",
    });

    const outputName = file.name.replace(/\.pdf$/i, ".docx") || "converted.docx";
    ...
    console.error("[pdf-to-word] Error:", msg);
    return NextResponse.json(
